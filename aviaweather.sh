...
parse_temperature() {
    ...
    # Расчет вероятности тумана - ИСПРАВЛЕННАЯ ВЕРСИЯ
    local temp_num temp_abs dew_num dew_abs
    ...
    # Убираем ведущие нули, но сохраняем отрицательные значения
    temp_num=$(echo "$temp_num" | sed 's/^0*//')
    dew_num=$(echo "$dew_num" | sed 's/^0*//')
    
    # Если после удаления нулей строка пустая, устанавливаем 0
    temp_num=${temp_num:-0}
    dew_num=${dew_num:-0}
    
    # Для отрицательных чисел добавляем минус обратно
    if [[ "$temp" == -* ]] || [[ "$temp" == M* ]]; then
        if [[ "$temp_num" != "-"* ]]; then
            temp_num="-$temp_num"
        fi
    ...
    # Преобразуем в целые числа для вычислений
    temp_num=$((temp_num))
    dew_num=$((dew_num))
